...
switch($method) {
    case 'GET':
        ...
        if(isset($id)) {
            $stmt = $conn->prepare("SELECT p.*, (SELECT GROUP_CONCAT(CONCAT(p2.lastName,' ',p2.firstName) SEPARATOR ', ') FROM person p2 WHERE p2.mainPersonId = p.mainPersonId and p2.og = p.og) relatedNames, (SELECT count(*) FROM course_history ch WHERE ch.personId = p.id) courseHistoryCount FROM person p WHERE p.og=? and p.id=?");
            $stmt->bind_param("si", $og, $id);
        }
        else {
            $stmt = $conn->prepare("SELECT p.*, (SELECT GROUP_CONCAT(CONCAT(p2.lastName,' ',p2.firstName) SEPARATOR ', ') FROM person p2 WHERE p2.mainPersonId = p.mainPersonId and p2.og = p.og) relatedNames, (SELECT count(*) FROM course_history ch WHERE ch.personId = p.id) courseHistoryCount FROM person p WHERE p.og=?");
